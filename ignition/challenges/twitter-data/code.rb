...
#YOUR CODE GOES HERE
all_the_data = twitter_data  #The array that will contain all Twitter info

#Question #1
puts "1. Each username followed by its description. If there is no description (i.e.,
    `nil` then print out \"NA\" instead):\n\n"

all_the_data.each do |hash|
  if hash[hash.keys.first]["description"].nil?  #If an explicit decsription was not written
    puts "#{hash.keys.first}: NA"              #put NA
  else                                         #Otherwise, output the description
    puts "#{hash.keys.first}: #{hash[hash.keys.first]["description"]}"
  end
end
puts "\n\n\n\n"
...
#Question #2
puts "2. Each username followed by total number of followers.\n\n"
all_the_data.each do |hash|
  puts "#{hash.keys.first}: #{hash[hash.keys.first]["number of followers"]}"
end
puts "\n\n\n\n"

#Question #3
puts "3. Each username and the length of their latest tweet.\n\n"
all_the_data.each do |hash|
  puts "#{hash.keys.first}'s latest tweet was #{hash[hash.keys.first]["latest tweet"].size} characters long"
end
puts "\n\n\n\n"

#Question #4
puts "4. Each username followed by the total character count used in that user's last
    twenty tweets.\n\n"
sum = 0
all_the_data.each do |hash|
  sum = 0             #Create a running sum
  hash[hash.keys.first]["last twenty tweets"].each do |tweet|
    sum += tweet.size   #For each individual tweet, add the # of characters to sum
  end
...
#Question #5
                    ...
                    #Initializations of key variables used to answer question
usernames_with_description_list = []; usernames_with_location_list = []
max_num_followers = -1; max_num_friends = -1; max_num_tweets = -1
max_followers_username = ""; max_friends_username = "", max_tweets_username = ""

all_the_data.each do |hash|
  hash.each do |username, info|      #Used for Question 5a.
    if info["number of followers"] > max_num_followers   #Iterates and compares values to
        max_num_followers = info["number of followers"]  #determine maximum values
        max_followers_username = username
    end
    if info["number of friends"] > max_num_friends       #Used for Question 5b.
        max_num_friends = info["number of friends"]      #Identical purpose as directly above (5a)
        max_friends_username = username
    end
    if info["number of tweets"] > max_num_tweets         #Used for Question 5c.
      max_num_tweets = info["number of tweets"]          #Identical purpose as directly above (5a)
      max_tweets_username = username
    end
    if !hash[hash.keys.first]["description"].nil?     #Used for Question 5d
      usernames_with_description_list << hash.keys.first
    end
    if !hash[hash.keys.first]["location"].nil?        #Used for Question 5e
      usernames_with_location_list << hash.keys.first
    end
  end
end


                              #Outputs the answers to 5a through 5e
puts "5a. Which user has the most followers?\n\n"
puts "#{max_followers_username} has the most followers with #{max_num_followers} total followers."
puts "\n\n\n\n"
puts "5b. Which user has the most friends?\n\n"
puts "#{max_friends_username} has the most friends with #{max_num_friends} total friends."
puts "\n\n\n\n"
puts "5c. Which user has the greatest number of tweets?\n\n"
puts "#{max_tweets_username} has the greatest number of tweets with #{max_num_tweets} total tweets."
puts "\n\n\n\n"
puts "5d. Which users have a `description` listed in `twitter_data`?\n\n"
puts "The following uers have a listed description:"
puts usernames_with_description_list
puts "\n\n\n\n"
puts "5e. Which users have a `location` listed in `twitter_data`?\n\n"
puts "The following uers have a listed location:"
puts usernames_with_location_list
